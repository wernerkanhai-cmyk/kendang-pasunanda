...
function App() {
  ...
  // Default is sunda-vox-werner.
  const [voicePackId, setVoicePackId] = useState(() => localStorage.getItem('kendangVoicePackId') || 'sunda-vox-werner');
  useEffect(() => {
    if (voicePackId) localStorage.setItem('kendangVoicePackId', voicePackId);
    else localStorage.removeItem('kendangVoicePackId');
  }, [voicePackId]);

  // Welke notatie-font + glyph-mapping. Default neodamina-werner.
  const [notationPackId, setNotationPackId] = useState(() => localStorage.getItem('kendangNotationPackId') || 'neodamina-werner');
  useEffect(() => {
    localStorage.setItem('kendangNotationPackId', notationPackId);
  }, [notationPackId]);

  // Beschikbare packs voor de selector-UI (uit public/packs/index.json).
  const [availablePacks, setAvailablePacks] = useState({ instrument: [], voice: [], notation: [] });
  useEffect(() => {
    let cancelled = false;
    packRegistry.discoverAll()
      .then(g => { if (!cancelled) setAvailablePacks(g); })
      .catch(e => errorLog.warn('App', 'pack discovery failed', e?.message || String(e)));
    return () => { cancelled = true; };
  }, []);

  // Notation pack laden + font registreren bij wisseling.
  useEffect(() => {
    if (!notationPackId) return;
    let cancelled = false;
    (async () => {
      try {
        const np = await packRegistry.load(notationPackId);
        if (cancelled) return;
        loadNotationFont(np);
        setNotationPack(np);
      } catch (e) {
        errorLog.error('App', 'notation pack load failed', `${notationPackId} — ${e?.message || String(e)}`);
      }
    })();
    return () => { cancelled = true; };
  }, [notationPackId]);
  ...
  useEffect(() => {
    ...
    // Default instrument pack laden. Notation en voice worden door eigen
    // effecten (op notationPackId / voicePackId) afgehandeld.
    (async () => {
      try {
   ...
   * Regels MÉT actieve automation (punten + enabled) spelen hun eigen curve. Alle
   * andere slots — OFF-regels of regels zonder punten — spelen op het GLOBALE tempo
   * (defaultBpm), zodat ze live met de BPM-knop mee veranderen. (Geen carry-over van
   * het vorige regel-tempo: dat gaf een 'vast' tempo dat je niet kon bijregelen en
   * botste met de grijze global-weergave.)
   */
  const buildTempoAt = (songArr, defaultBpm) => {
    return (globalSlot) => {
      ...
      return defaultBpm; // voorbij de laatste regel — globaal tempo
    };
  };
            ...
            {showToolsMenu && (
              <>
                ...
                <div style={{
                  ...
                  ><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>{t('exportPdf')}</button>
                  <button
                    onClick={() => setShowPdfSettings(v => !v)}
                    style={{ background: '#0f172a', color: '#94a3b8', border: '1px solid #334155', borderRadius: '6px', padding: '0.5rem 0.8rem', textAlign: 'left', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.45rem' }}
                  ><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="18" x2="20" y2="18"/><circle cx="8" cy="6" r="2"/><circle cx="16" cy="12" r="2"/><circle cx="9" cy="18" r="2"/></svg>{t('pdfLayoutSettings')}</button>
                  ...
                  {showPdfSettings && (
                    <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', padding: '0.7rem', fontSize: '0.8rem', color: '#e2e8f0' }}>
                      ...
                      ><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: '-2px', marginRight: '5px' }}><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>{t('restoreDefaults')}</button>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    style={{ background: '#0f172a', color: '#e2e8f0', border: '1px solid #334155', borderRadius: '6px', padding: '0.5rem 0.8rem', textAlign: 'left', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.45rem' }}
                  ><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>{t('manual')}</button>
        ...
        >
          ...
          <div style={{ flex: 1, overflow: 'auto', minHeight: 0, display: drumCollapsed ? 'none' : 'block' }}>
            <DrumPad
              ...
              onNotationPackChange={setNotationPackId}
              voicePackId={voicePackId}
              onVoicePackChange={setVoicePackId}
            />
          </div>
